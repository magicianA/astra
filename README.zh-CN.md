...
在“显示 → 曝光”中启用“自适应曝光”，可按当前画面随拖动、缩放平滑调整亮度，
放大月亮时自动保留月面细节，暗夜也不会被强行抬成灰色。启用时曝光补偿归零，
之后仍可用滑块调节。此模式优先于原有的全天自动曝光，开关会随场景保存。
示例：`examples/adaptive-moon.json`。
该示例使用开启大气的夜间观月场景。如果自定义场景在白天仍是一片黑色，
点击“时间 → 恢复昼夜变化”，即可启用大气与自动曝光，并将曝光补偿恢复为 0 EV。
跳转黎明或黄昏也会恢复这些设置。

![暮光中的星空：银河与成片星点，地平线上仍有橙红色余晖](docs/images/dusk.png)

*北京，2026 年 9 月 8 日 19:28，当地平太阳时：银河、星点与黄昏余晖同框。*

![阿塔卡马沙漠上空的银河与恒星](docs/images/milky-way.png)

*阿塔卡马沙漠，2026 年 6 月 15 日 00:00，当地平太阳时。*

两张图片均由 macOS 上的 Vulkan 渲染器直接导出，原始分辨率为 2880 × 1800，
隐藏全部 UI 和文字标注，未经后期处理。[场景文件与截图命令](docs/images/README.md)可用于复现。
...
左侧「探索」加入月面地形与阴影、地球照、88 星座导览、双年代并排对照、24 小时观测计划、地平线轮廓导入、定步长视频与星轨导出，以及日月食、掩星和天体接近搜索。
月面细节可直接打开 `examples/moon-detail.json`；另有双年代、山景和日月食示例。月球数值姿态覆盖约 1550—2650 年，范围外明确使用 IAU 近似。MP4 需要额外安装 FFmpeg（本机已安装），PNG 和星轨不依赖它。
具体操作、数据来源和精度边界见 [探索功能说明](docs/EXPLORATION.md)。搜索也已改为索引查找，不再受可见星等阈值限制；旧场景迁移会提示，未知模型版本会拒绝载入。
...
严格的 Johnson V；渲染已加入 Gaia G 到 V 的近似转换并统一光度单位，但混合波段、颜色代理和显示映射仍有误差。星表的 `G ≤ 12` 是目录历元的筛选，不能保证远期
